...
sshpass -p ${PASSWORD} ssh -o StrictHostKeyChecking=no ${USER}@${IP} "service iofog-agent stop; sudo apt-get update; sudo apt-get install --only-upgrade -y iofog-agent${DEV}"
